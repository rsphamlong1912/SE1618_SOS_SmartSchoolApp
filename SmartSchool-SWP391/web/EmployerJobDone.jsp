                    ...
                    <c:if test="${!empty requestScope.MY_JOB_POST_DONE}">
                        <c:forEach items="${requestScope.MY_JOB_POST_DONE}" var="myJobPostDone">
                            <div class="card w-100">
                                <div class="card-body">
                                    <h5 class="card-title mb-3">${myJobPostDone.title}</h5>
                                    <p class="card-text"><i class="fa fa-clock" aria-hidden="true"></i> ${myJobPostDone.date}</p>
                                    <div class="row mb-3">
                                        <p class="col-md-4 card-text">Lĩnh vực: ${myJobPostDone.jobCategoryName} </p>
                                        <p class="col-md-4 card-text">Yêu cầu số người:  ${myJobPostDone.amount} </p>
                                        <!--<p class="col-md-4 card-text">Thời gian: 1-3 tháng </p>-->
                                        <p class="col-md-4 card-text">
                                            Thời gian: 
                                            <c:choose> 
                                                <c:when test="${myJobPostDone.timeJob==1}">
                                                    Ít hơn 1 tháng
                                                </c:when> 
                                                <c:when test="${myJobPostDone.timeJob==2}">
                                                    1 - 3 tháng
                                                </c:when>
                                                <c:when test="${myJobPostDone.timeJob==3}">
                                                    Hơn 3 tháng
                                                </c:when> 
                                            </c:choose>
                                        </p>
