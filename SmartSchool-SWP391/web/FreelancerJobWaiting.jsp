    ...
    <body>
        ...
        <main class="main" id="top">
            ...
            <section style="padding: 50px 0 ;min-height: 85vh">
                <div class="container">
                    <c:if test="${empty requestScope.MY_JOB_WAITING}">
                        <h1 class="text-center">${ERROR}</h1>
                    </c:if>
                    <c:if test="${!empty requestScope.MY_JOB_WAITING}">
                        <c:forEach items="${requestScope.MY_JOB_WAITING}" var="myJobPostWaiting" varStatus="count">
                            <div class="card w-100">
                                <div class="card-body">
                                    <h5 class="card-title mb-3">${myJobPostWaiting.title}</h5>
                                    <p class="card-text"><i class="fa fa-clock" aria-hidden="true"></i> ${myJobPostWaiting.date}</p>
                                    <div class="row mb-3">
                                        <p class="col-md-4 card-text">Lĩnh vực: ${myJobPostWaiting.jobCategoryName} </p>
                                        <p class="col-md-4 card-text">Yêu cầu số người:  ${myJobPostWaiting.amount} </p>
                                        <!--<p class="col-md-4 card-text">Thời gian: 1-3 tháng </p>-->
                                        <p class="col-md-4 card-text">
                                            Thời gian: 
                                            <c:choose> 
                                                <c:when test="${myJobPostWaiting.timeJob==1}">
                                                    Ít hơn 1 tháng
                                                </c:when> 
                                                <c:when test="${myJobPostWaiting.timeJob==2}">
                                                    1 - 3 tháng
                                                </c:when>
                                                <c:when test="${myJobPostWaiting.timeJob==3}">
                                                    Hơn 3 tháng
                                                </c:when> 
                                            </c:choose>
                                        </p>
                                    ...
                                    <div class="row justify-content-end">
                                        <a href="#" class="col-md-2 btn btn-outline-primary mt-2" class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapse${count.index + 1}" aria-expanded="true" aria-controls="panelsStayOpen-collapse${count.index + 1}">Xem câu hỏi đăng ký</a>
                                        <a href="main?jobId=${myJobPostWaiting.jobId}&userId=${myJobPostWaiting.userId}&action=DetailJob" class="col-md-2 btn btn-primary gradient-custom-2 ms-3 mt-2">Xem chi tiết</a>
                                    </div>
                                    <div id="panelsStayOpen-collapse${count.index + 1}" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-heading${count.index + 1}">
                                        <div class="accordion-body">
                                            <h5 class="card-title mb-3">Câu hỏi đăng ký công việc</h5>
                                             <c:forEach items="${requestScope.MY_JOB_ANSWER_WAITING}" var="answerQuestion">
                                                 <c:if test="${myJobPostWaiting.jobId == answerQuestion.jobId}">
                                            <div class="mb-4">
                                                
                                                <label for="formGroupExampleInput" class="form-label fw-bold">${answerQuestion.question}</label>
                                                <input type="hidden" class="form-control" id="formGroupExampleInput" name="idQuestion" value="">                                                    
